...
RdmaResource *RdmaResourceFactory::GetRdmaResource(bool isMaster, const char *devName)
{
  ...
  if (devName)
  {
    for (std::vector<RdmaResource *>::iterator it = resources.begin();
         it != resources.end(); ++it)
    {
      if (!strcmp((*it)->GetDevname(), devName) && (*it)->IsMaster() == isMaster)
        return (*it);
    }
  }
      ...
      pending_send_msg(0)
{
  ...
  max_unsignaled_msg =
      MAX_UNSIGNALED_MSG > max_pending_msg ? max_pending_msg : MAX_UNSIGNALED_MSG;
...

  {
    ...
    qattr.qp_access_flags = IBV_ACCESS_LOCAL_WRITE | IBV_ACCESS_REMOTE_WRITE | IBV_ACCESS_REMOTE_ATOMIC | IBV_ACCESS_REMOTE_READ;
                          ...
                          uint32_t imm, uint64_t oldval, uint64_t newval)
{
  ...
  if (pending_msg >= max_pending_msg)
  {
    ...
    epicAssert(
        pending_requests.back().op == op && pending_requests.back().src == src && pending_requests.back().len == len && pending_requests.back().id == id && pending_requests.back().signaled == signaled && pending_requests.back().dest == dest && pending_requests.back().imm == imm && pending_requests.back().oldval == oldval && pending_requests.back().newval == newval);
  ...
  if (unlikely(curr_to_signaled_send_msg + curr_to_signaled_w_r_msg == max_unsignaled_msg || signaled))
...
void RdmaContext::ProcessPendingRequests(int n)
{
  ...
  for (i = 0; i < size; i++)
  {
    ...
    if (r.op == IBV_WR_SEND)
    {
      ...
      if (prev_wr && prev_wr->opcode == IBV_WR_SEND && buf_pos + r.len + 1 + 1 <= MAX_REQUEST_SIZE && !r.signaled)
      {
        /* add ergeda add */
        // 改了上面的if
        /* add ergeda add */
    ...
    else if (r.op == IBV_WR_RDMA_WRITE || r.op == IBV_WR_RDMA_WRITE_WITH_IMM)
    {
      j++;
      if (j == n)
      {
    ...
    uint16_t curr_to_signaled_w_r_msg = pending_msg - pending_send_msg - to_signaled_w_r_msg;
    if (unlikely(
            curr_to_signaled_send_msg + curr_to_signaled_w_r_msg == max_unsignaled_msg || r.signaled))
    {
      ...
      epicAssert(to_signaled_send_msg + to_signaled_w_r_msg == pending_msg);
      epicAssert(
          curr_to_signaled_send_msg + curr_to_signaled_w_r_msg <= max_unsignaled_msg);
      wrs[j].wr_id = (r.id & HALF_BITS) + ((uint64_t)(curr_to_signaled_send_msg & QUARTER_BITS) << 48) + ((uint64_t)(curr_to_signaled_w_r_msg & QUARTER_BITS) << 32);
    }
