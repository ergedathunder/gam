...
void sub_fft(GAddr addr_value, unsigned int n, unsigned int l, unsigned int k, Complex T, GAlloc *alloc_thread)
{
    for (unsigned int a = l; a < no_array; a += n)
    {
        ...
        alloc_thread->Read(addr_value + a * sizeof(complex<float>), (int *)&xa, sizeof(complex<float>));
        // Read_val(Cur_wh, addr_value + b * sizeof(complex<float>), (int *)&xb, sizeof(complex<float>));
        alloc_thread->Read(addr_value + b * sizeof(complex<float>), (int *)&xb, sizeof(complex<float>));
        Complex t = xa - xb;
        xa = xa + xb;
        // Write_val(Cur_wh, addr_value + a * sizeof(complex<float>), (int *)&xa, sizeof(complex<float>), 1);
        alloc_thread->Write(addr_value + a * sizeof(complex<float>), (int *)&xa, sizeof(complex<float>), 1);
        xb = t * T;
        // Write_val(Cur_wh, addr_value + b * sizeof(complex<float>), (int *)&xb, sizeof(complex<float>), 1);
        alloc_thread->Write(addr_value + b * sizeof(complex<float>), (int *)&xb, sizeof(complex<float>), 1);
    }
}
...
{
    GAlloc *alloc_thread = GAllocFactory::CreateAllocator();

    printf("alloc->getid=%d,alloc_thread->getid=%d\n", alloc->GetID(), alloc_thread->GetID());
    int interval_worker = worker_num;
    int interval_thread = thread_num;
    int interval = interval_worker * interval_thread;
    for (int index = start_index + thread_id * interval_worker; index < k; index += interval)
    {
        sub_fft(addr_value, n, index, k, T, alloc_thread);
        // T *= phiT;
...
void sub_fft_perworker(GAddr addr_value, unsigned int n, unsigned int start_index, unsigned int k, Complex T, Complex phiT)
{
    ...
    // for (int index = start_index; index < k; index += interval)
    // {
    //     sub_fft(addr_value, n, index, k, T);
    //     // T *= phiT;
...
void p_fft_RC(GAddr addr_value)
{
    ...
    while (k > 1)
    {
        ...
        alloc->acquireLock(1, addr_value, sizeof(complex<float>) * no_array, true, sizeof(complex<float>));

        // for (unsigned int l = 0; l < k; l++)
        // {
        //     int id_parr = l % worker_num + 1;
        //     if (id_parr == node_id)
        //     {
        ...
        id = 0;
        alloc->Put(SYNC_KEY * 4 + node_id + k * 197, &node_id, sizeof(int));
        for (int i = 1; i <= no_node; i++)
        {
            alloc->Get(SYNC_KEY * 4 + i + k * 197, &id);
            epicAssert(id == i);
        }
    }
...
void Solve_RC()
{
    ...
    if (is_master)
    {
        ...
        addr_value = alloc->AlignedMalloc(sizeof(complex<float>) * no_array, RC_Write_shared, 1);
        printf("addr_value=%lx\n", addr_value);
        // alloc->Write(addr_value, (int *)value, sizeof(complex<float>) * no_array);
        alloc->Write(addr_value, (int *)value.data(), sizeof(complex<float>) * no_array);
    ...
    if (is_master)
    {
        ...
        if (is_read)
        {
            ...
            // alloc->Read(addr_value, (int *)readbuf, sizeof(complex<float>) * no_array);
            alloc->Read(addr_value, (int *)readbuf.data(), sizeof(complex<float>) * no_array);
...
void Solve_MSI()
{
    ...
    if (is_master)
    {
        ...
        addr_value = alloc->AlignedMalloc(sizeof(complex<float>) * no_array, Msi, 1);
