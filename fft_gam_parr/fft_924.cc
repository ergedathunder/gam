...
void Synchro()
{
    ...
    for (int i = 0; i < thread_num; i++)
    {
        ...
        printf("after use key %ld\n", SYNC_KEY + node_id * no_node + i);
    }
    ...
    for (int i = 1; i <= no_node; i++)
    {
        ...
        {
            alloc_array[j]->Get(SYNC_KEY + i * no_node + j, &id);
        }
    }

    SYNC_KEY = SYNC_KEY + no_node * thread_num + 1;
}
...
void Solve_RC()
{
    ...
    for (int alloc_id = 0; alloc_id < begin_thread_num; alloc_id++)
    {
...
int main(int argc, char *argv[])
{
    ...
    else
    {
        ...
        int alloc_complete_flag[no_node];
        for (int i = 1; i <= no_node; i++)
        {
            if (i == node_id)
            {
                alloc_complete_flag[i] = 923 + i;
                alloc_array[0]->Put(2023 + i, &alloc_complete_flag[i], sizeof(int));
            }
        }
