...
void dit_r2_fft(GAddr addr_xn, int n, int stride, GAddr addr_Xk, WorkerHandle *Cur_wh)
{
    ...
    GAddr addr_X1k = Malloc_addr(Cur_wh, sizeof(complex<float>) * n / 2, Write_shared, 1);
    GAddr addr_X2k = Malloc_addr(Cur_wh, sizeof(complex<float>) * n / 2, Write_shared, 1);
...
void Solve()
{
    ...
    GAddr addr_xn = Malloc_addr(wh[0], sizeof(float) * N, Write_shared, 1);
    for (int i = 0; i < N; i++)
    {
        Write_val(wh[0], addr_xn + i * sizeof(float), (int *)&xn[i], sizeof(float));
    }

    GAddr addr_Xk = Malloc_addr(wh[0], sizeof(complex<float>) * N, Write_shared, 1);
